...
Shared protocol for executing a flow task through its workflow steps. Referenced by `/flow:run`, `/flow:task-create --run`, and `/flow:go`.

## Prerequisites

Before starting the execution loop, resolve these values:

- **taskId**: The task ID to execute
- **taskDir**: `${CLAUDE_CODE_TASK_LIST_ID}` if set, otherwise `${CLAUDE_SESSION_ID}`
- **taskFilePath**: `~/.claude/tasks/${taskDir}/{taskId}.json`
- **autonomy**: Whether to set `autonomy: true` on Navigate calls (auto-continue through stage boundaries)

When `autonomy` is enabled, the navigator auto-continues through stage boundary end nodes (end nodes with outgoing `on: "passed"` edges to non-terminal nodes) instead of stopping at each one. When `autonomyContinued` is `true` in the Navigate response, log the stage transition:
...
**If subagent is set (e.g., `Developer`):**
...
```
✓ parse_requirements (Planner) → passed
✓ implement (Developer) → passed
⟳ test (Tester) → in progress...
```
...
- `subagent` — Task tool `subagent_type` value (e.g., `"Developer"`)
...
```
Completed: #1 Add user auth ✨ (Developer)

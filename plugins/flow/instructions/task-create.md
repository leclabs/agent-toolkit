...
Shared protocol for creating a flow task from a description. Referenced by `/flow:task-create` and `/flow:task`.
...
```
TaskCreate(
  subject: `#{taskId} Add user authentication ✨
→ feature-development · parse_requirements (Planner)`,
  activeForm: "Parse Requirements (Planner)",
...
#3 Add user auth ✨
→ feature-development · code_review (Reviewer) · retries: 0/2
...
If `--run` flag is provided, execute the task immediately after creation using the resolved `taskId` from step 2, `autonomy: false`.
